...
# GPU side of the Qwen3-0.6B Megatron SFT alignment run.
...
RUN_NAME="qwen3_0.6b_megatron_sft_gpu_$(date +%Y%m%d_%H%M%S)"
...
nvidia-smi > "${RUN_DIR}/device_info.txt" 2>&1 || true
cp "$0" "${RUN_DIR}/run_script.sh" 2>/dev/null || true

echo "Starting GPU Megatron SFT alignment run: ${RUN_NAME}"
...
echo "GPU alignment run completed: ${RUN_DIR}"
